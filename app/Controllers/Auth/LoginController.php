...
class LoginController extends Controller
{
    ...
    {
        $this->middleware('guest')->except('logout'); // Re-enable for standard behavior
    }
    ...
    public function login(Request $request)
    {
        ...
        dd('Credentials received:', $credentials); // Debug point 1

        if (Auth::attempt($credentials)) {
            $request->session()->regenerate();
            Log::info('Login successful for user:', ['username' => $request->username]);
            dd('Login successful, redirecting to: ' . $this->redirectTo); // Debug point 2
            return redirect($this->redirectTo);
        }

        Log::warning('Login failed for user:', ['username' => $request->username]);
        dd('Login failed, returning to login page.'); // Debug point 3
