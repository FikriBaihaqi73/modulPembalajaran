        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
            <div class="flex justify-between items-center p-5 border-b border-gray-100">
                ...
                @if ($notifications->count() > 0)
                    <form action="{{ route('santri.notifications.markAllAsRead') }}" method="POST">
                        @csrf
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div>
                                            @if ($notification->type === 'App\\Notifications\\ReviewReplied')
                                                <p class="font-medium text-gray-900">{{ $notification->data['replier_name'] ?? 'Sistem' }}</p>
                                                <p class="text-gray-700 text-sm mt-1">Membalas ulasan Anda pada modul <a href="{{ $notification->data['link'] }}" class="text-blue-600 hover:underline font-medium">{{ $notification->data['module_name'] }}</a></p>
                                        ...
                                        <div class="flex items-center space-x-2">
                                            @if (!$notification->read_at)
                                                <form action="{{ route('santri.notifications.markAsRead', $notification->id) }}" method="POST">
                                                    @csrf
                                            ...
                                            @else
                                                <form action="{{ route('santri.notifications.markAsUnread', $notification->id) }}" method="POST">
                                                    @csrf
                                                ...
                                                <form action="{{ route('santri.notifications.destroy', $notification->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus notifikasi ini?');">
                                                    @csrf
                                                    @method('DELETE')
