    ...
    <div class="mt-4 bg-white p-6 rounded shadow w-full">
        <form action="{{ route('mentor.modules.store') }}" method="POST" enctype="multipart/form-data" id="moduleForm">
            ...
            <div class="mb-4">
                ...
                <div id="module-categories-container" data-options='<option value="">Pilih Kategori Modul</option>@foreach($moduleCategories as $category)<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach'>
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('mentor.modules.index') }}" class="inline-block align-baseline font-bold text-sm text-blue-600 hover:text-blue-800" onclick="localStorage.removeItem('tiptapEditorContent'); localStorage.removeItem('lastActiveModulePage');">
                    Batal
                </a>
            </div>
        </form>
