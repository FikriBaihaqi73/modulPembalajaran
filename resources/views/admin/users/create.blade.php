...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <h2 class="text-2xl font-semibold mb-4">Tambah Pengguna Baru</h2>

        <form action="{{ route('admin.users.store') }}" method="POST" class="max-w-md bg-white p-6 rounded shadow">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="username" id="username" value="{{ old('username') }}" class="w-full border rounded px-3 py-2 @error('username') border-red-500 @enderror" required>
                    @error('username')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div id="major-field" style="display: {{ (old('role_id') && in_array(App\Models\Role::find(old('role_id'))->name ?? '', ['Mentor', 'Santri'])) ? 'block' : 'none' }};">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initial call to set visibility based on old input or default
            toggleMajorField();
        });
    </script>
@endsection
